	...
	let tls_config = match (&tls_private_key_der, &tls_certificate_chain_der) {
		(Some(key_der), Some(cert_chain_der)) if !cert_chain_der.is_empty() => {
			...
			if let Err(e) = builder.server(key, certs) {
				...
				// 自签证书不是节点身份真源；仍由 rustls provider 验证 CertificateVerify 签名，
				// 后续 Noise 握手按 multiaddr 固定 peer ID 验证节点身份。
				let provider = futures_rustls::rustls::crypto::ring::default_provider();
				let verifier = NoiseAuthenticatedCertVerifier {
					supported: provider.signature_verification_algorithms,
				};
				match futures_rustls::rustls::ClientConfig::builder_with_provider(Arc::new(
					provider,
				))
				.with_safe_default_protocol_versions()
				{
					Ok(builder) => {
						let client = builder
							...
							.with_no_client_auth();
						config.client = Arc::new(client).into();
					},
					Err(error) => {
						log::warn!("TLS 客户端协议配置失败，将保留 Web PKI 客户端配置: {error}");
					},
				}
...
struct NoiseAuthenticatedCertVerifier {
	supported: futures_rustls::rustls::crypto::WebPkiSupportedAlgorithms,
}

impl futures_rustls::rustls::client::danger::ServerCertVerifier for NoiseAuthenticatedCertVerifier {
	...
	fn verify_tls12_signature(
		&self,
		message: &[u8],
		cert: &futures_rustls::rustls::pki_types::CertificateDer<'_>,
		signature: &futures_rustls::rustls::DigitallySignedStruct,
	) -> Result<
		futures_rustls::rustls::client::danger::HandshakeSignatureValid,
		futures_rustls::rustls::Error,
	> {
		futures_rustls::rustls::crypto::verify_tls12_signature(
			message,
			cert,
			signature,
			&self.supported,
	...
	fn verify_tls13_signature(
		...
		futures_rustls::rustls::Error,
	> {
		futures_rustls::rustls::crypto::verify_tls13_signature(
			message,
			cert,
			signature,
			&self.supported,
